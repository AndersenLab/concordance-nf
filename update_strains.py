    ...
    for key, value in kwargs.items():
        if type(value) == str:
            ...
            if key in m:
                ...
                m[key] = value
            try:
                value = [x for x in value if not "BGI-20151012-ECA23" in x]
            except:
                pass
            print(value)
...
for fq in fastqs:
    ...
    if 'seq_folder' in fq.keys():
        if fq['seq_folder'] != "original_wi_seq":
            if 'library' not in fq.keys():
                fq['library'] = fq['barcode'].replace("+", "").replace("_", "")
            if fq['library'] != fq['barcode'].replace("+", "").replace("_", ""):
